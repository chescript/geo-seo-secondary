...
export async function performAnalysis({
  ...
  let analysisPrompts;
  console.log('🎯 [ANALYZE-COMMON] Checking prompts:', {
    hasCustomPrompts: !!customPrompts,
    customPromptsLength: customPrompts?.length || 0,
    willGeneratePrompts: !customPrompts || customPrompts.length === 0
  });

  if (customPrompts && customPrompts.length > 0) {
    console.log('✅ [ANALYZE-COMMON] Using custom prompts:', customPrompts);
    ...
    console.log('🤖 [ANALYZE-COMMON] Generating prompts with AI...');
    console.log('🤖 [ANALYZE-COMMON] Company data:', {
      name: company.name,
      industry: company.industry,
      description: company.description?.substring(0, 100),
      hasScrapedData: !!company.scrapedData
    });
    console.log('🤖 [ANALYZE-COMMON] Competitors:', competitors);

    const prompts = await generatePromptsForCompany(company, competitors);

    console.log('✅ [ANALYZE-COMMON] Prompts generated:', {
  ...
  console.log('📋 [ANALYZE-COMMON] Final analysis prompts:', analysisPrompts.map(p => p.prompt));

  // Send prompt generated events
  for (let i = 0; i < analysisPrompts.length; i++) {
    console.log(`📤 [ANALYZE-COMMON] Sending prompt-generated event ${i + 1}/${analysisPrompts.length}:`, analysisPrompts[i].prompt);
    await sendEvent({
  ...
  // Filter providers based on available API keys and user selection
  const allAvailableProviders = getAvailableProviders();
  const availableProviders =
    selectedProviders && selectedProviders.length > 0
      ? allAvailableProviders.filter((provider) =>
  ...
  console.log(
    ...
    allAvailableProviders.map((p) => p.name)
  );
  console.log(
    "Filtered providers for analysis:",
    availableProviders.map((p) => p.name)
  );
  console.log(
    "Available provider details:",
    availableProviders.map((p) => ({ name: p.name, model: p.model }))
  );
  ...
  for (
    let batchStart = 0;
    batchStart < analysisPrompts.length;
    ...
    const batchPromises = batchPrompts.flatMap((prompt, batchIndex) =>
      availableProviders.map(async (provider) => {
        ...
        try {
          ...
          console.log(
            `Attempting analysis with provider: ${provider.name} for prompt: "${prompt.prompt.substring(0, 50)}..."`
          );

          // Choose the appropriate analysis function based on useWebSearch
          const analyzeFunction = useWebSearch
          ...
          console.error(
            `Error with ${provider.name} for prompt "${prompt.prompt}":`,
            error
          );
          errors.push(
  ...
  const competitorRankings = await analyzeCompetitors(
    company,
    responses,
    competitors
  );
  ...
  const { providerRankings, providerComparison } =
    await analyzeCompetitorsByProvider(company, responses, competitors);

  // Calculate final scores
  const scores = calculateBrandScores(
    responses,
    company.name,
    competitorRankings
  );
