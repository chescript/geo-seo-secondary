...
const PromptSchema = z.object({
  prompts: z.array(z.object({
    prompt: z.string().describe('A search query that people might ask AI assistants about companies in this industry'),
    category: z.enum(['ranking', 'comparison', 'alternatives', 'recommendations']).describe('Category of the prompt'),
    reasoning: z.string().describe('Why this prompt is relevant for this company and industry')
  }))
});

export async function generatePromptsForCompany(company: Company, competitors: string[]): Promise<BrandPrompt[]> {
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  console.log('🤖 [PROMPTS] START: Generating AI-powered prompts');
  console.log('📋 [PROMPTS] Input data:', {
    companyName: company.name,
    industry: company.industry,
    description: company.description?.substring(0, 100) + '...',
    mainProducts: company.scrapedData?.mainProducts,
    keywords: company.scrapedData?.keywords?.slice(0, 5),
  ...
  try {
    ...
    console.log('🔍 [PROMPTS] Step 1: Checking configured providers...');
    const configuredProviders = getConfiguredProviders();
    console.log(`📊 [PROMPTS] Found ${configuredProviders.length} configured providers:`,
      configuredProviders.map(p => p.name));

    if (configuredProviders.length === 0) {
      console.warn('⚠️  [PROMPTS] No AI providers configured, using fallback prompts');
      const fallback = generateFallbackPrompts(company, competitors);
      console.log('🔄 [PROMPTS] Returning fallback prompts:', fallback.map(p => p.prompt));
      return fallback;
    }

    // Step 2: Select model
    console.log('🔍 [PROMPTS] Step 2: Selecting fast model...');
    const provider = configuredProviders[0];
    console.log(`📍 [PROMPTS] Using provider: ${provider.name}`);
    console.log(`📍 [PROMPTS] Available models:`, provider.models.map(m => m.name));
    ...
    console.log(`🚀 [PROMPTS] Fast model found:`, fastModel?.name || 'NONE');

    const modelId = fastModel?.id || provider.defaultModel;
    console.log(`📍 [PROMPTS] Selected model ID: ${modelId}`);

    const model = getProviderModel(provider.id, modelId);
    console.log(`📍 [PROMPTS] Model object obtained:`, model ? `YES (${model.modelId})` : 'NO');

    if (!model) {
      console.warn(`⚠️  [PROMPTS] ${provider.name} model not available, using fallback`);
      const fallback = generateFallbackPrompts(company, competitors);
      console.log('🔄 [PROMPTS] Returning fallback prompts:', fallback.map(p => p.prompt));
      return fallback;
    }

    console.log(`✅ [PROMPTS] Using ${provider.name} (${model.modelId}) to generate prompts`);

    // Step 3: Construct prompt
    console.log('🔍 [PROMPTS] Step 3: Constructing LLM prompt...');
    const promptText = `Generate 6 diverse search prompts that people might ask AI assistants (like ChatGPT, Claude, Perplexity) when looking for companies like ${company.name}.

Company Information:
- Name: ${company.name}
- Industry: ${company.industry || 'technology'}
- Description: ${company.description}
${company.scrapedData?.mainProducts ? `- Main Products: ${company.scrapedData.mainProducts.join(', ')}` : ''}
${company.scrapedData?.keywords ? `- Keywords: ${company.scrapedData.keywords.slice(0, 5).join(', ')}` : ''}
- Known Competitors: ${competitors.slice(0, 3).join(', ')}

Generate prompts that:
1. Are natural questions people would actually ask
2. Cover different intent types: rankings, comparisons, alternatives, recommendations
3. Are specific to this industry (use actual product/service names, not generic terms)
4. Would likely mention ${company.name} or competitors in AI responses
5. Represent real user search behavior

Examples of GOOD prompts for Tesla:
- "Best electric vehicles for long-range driving in 2025"
- "Tesla Model Y vs Rivian R1S for families"
- "Most reliable EV brands with good charging infrastructure"

Examples of BAD prompts:
- "Best brands in 2025" (too generic)
- "Tesla products" (not a question)
- "What is Tesla" (not about brand comparison)

Generate 6 prompts covering: 2 ranking, 2 comparison, 1 alternatives, 1 recommendations.`;

    console.log('📝 [PROMPTS] Prompt length:', promptText.length, 'characters');
    console.log('📝 [PROMPTS] First 200 chars:', promptText.substring(0, 200));

    // Step 4: Call LLM
    console.log('🔍 [PROMPTS] Step 4: Calling generateObject API...');
    console.log('⏳ [PROMPTS] Waiting for AI response...');
    ...
    console.log(`✅ [PROMPTS] AI response received in ${duration}ms`);
    console.log('📦 [PROMPTS] Response structure:', {
      hasPrompts: !!object?.prompts,
      promptCount: object?.prompts?.length || 0,
      promptsPreview: object?.prompts?.slice(0, 2).map(p => ({
    ...
    console.log('🔍 [PROMPTS] Step 5: Parsing AI response...');
    const brandPrompts: BrandPrompt[] = object.prompts.map((p, index) => ({
      id: (index + 1).toString(),
      prompt: p.prompt,
      category: p.category,
    }));

    console.log('✅ [PROMPTS] Successfully generated AI prompts:', brandPrompts.map(p => p.prompt));
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');

    return brandPrompts;
  } catch (error) {
    console.error('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    console.error('❌ [PROMPTS] FAILED to generate AI prompts');
    console.error('❌ [PROMPTS] Error type:', error instanceof Error ? error.constructor.name : typeof error);
    console.error('❌ [PROMPTS] Error message:', error instanceof Error ? error.message : String(error));
    console.error('❌ [PROMPTS] Error stack:', error instanceof Error ? error.stack : 'N/A');
    console.error('❌ [PROMPTS] Full error object:', JSON.stringify(error, null, 2));
    console.log('🔄 [PROMPTS] Falling back to template-based prompts');

    const fallback = generateFallbackPrompts(company, competitors);
    console.log('🔄 [PROMPTS] Fallback prompts:', fallback.map(p => p.prompt));
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
...
function generateFallbackPrompts(company: Company, competitors: string[]): BrandPrompt[] {
  const brandName = company.name;
  const industry = company.industry || 'technology';
  const mainProduct = company.scrapedData?.mainProducts?.[0] || industry;

  const prompts = [
    { prompt: `Best ${industry} companies in 2025`, category: 'ranking' as const },
    { prompt: `Top ${industry} brands ranked by quality`, category: 'ranking' as const },
    { prompt: `${brandName} vs ${competitors[0] || 'competitors'} comparison`, category: 'comparison' as const },
    { prompt: `How does ${brandName} compare to other ${industry} companies`, category: 'comparison' as const },
    { prompt: `Alternatives to ${brandName} for ${mainProduct}`, category: 'alternatives' as const },
    { prompt: `Is ${brandName} worth it for ${industry}`, category: 'recommendations' as const },
  ];
